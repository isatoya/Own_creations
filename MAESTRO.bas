...
'variables para todo el proyecto
    ...
    Dim Fecha1, Fecha2, Fecha3, mes, Mes_texto, Año, mes_eliminar, dia As String
    Dim ruta, Ruta_Año, Ruta_Mes, Ruta_Audi As String
...
If ruta_maestroA <> "Falso" Then
        ...
        If Dir(Ruta_Mes & "\" & Año & mes & " MAESTRO ACTIVOS" & ".xlsx") <> "" Then
            Kill Ruta_Mes & "\" & Año & mes & " MAESTRO ACTIVOS" & ".xlsx"
End If
...
'----------------------------------- Empieza a trabajar con el archivo nuevo del maestro -----------------------------------
    ...
    ActiveWorkbook.SaveAs Ruta_Mes & "\" & Año & mes & " MAESTRO ACTIVOS" & ".xlsx"
    MAESTRO.Save
    MAESTRO.Close
    Application.AskToUpdateLinks = True
    
    'Crea las hojas
    Workbooks(Año & mes & " MAESTRO ACTIVOS" & ".xlsx").Activate
...
'----------------------------------- CREA HOJA DE ALTAS -----------------------------------
    ...
    Workbooks("BAJAS_ALTAS.XLSX").Activate
    Rows("1:1").AutoFilter
    Rows("1:1").AutoFilter Field:=5, Criteria1:="=Contratación"
    Columns("A:AB").Select
...
'----------------------------------- CREA HOJA DE BAJAS -----------------------------------
    ...
    For y = 2 To lastRow_A
        If ActiveSheet.Cells(y, "D").Value = mes_eliminar Then
            ActiveSheet.Cells(y, "C").Value = dia & "." & mes & "." & Año
        End If
    Next y
    Columns("D:D").Select
    Selection.Delete
...
'----------------------------------- Trae la hoja del archivo de la zhr929 -----------------------------------
 ...
 MsgBox "Por favor selecciene el archivo de la ZHR929 descargado anteriormente.", vbInformation
    ...
    If ruta_maestroA <> "Falso" Then
            ...
            Application.CutCopyMode = False
            Workbooks(Año & mes & " MAESTRO ACTIVOS" & ".xlsx").Save
            MAESTRO.Save
            MAESTRO.Close
    ...
    Next cell
    Range("A1").Value = "Número de personal"
    
    'Pone condicional de duplicados - los señala de rojo
    Columns("A:A").Select
...
'----------------------------------- ORGANIZA LA HOJA SALARIAL  -----------------------------------
    ...
    For Each cell In rng
        cell.Value = Val(cell.Value)
    Next cell
    Range("A1").Value = "Nº pers."
    
    
    'Condicional de duplicados - Los señala con rojo
    Columns("A:A").Select
    Selection.FormatConditions.AddUniqueValues
    Selection.FormatConditions(Selection.FormatConditions.Count).SetFirstPriority
    Selection.FormatConditions(1).DupeUnique = xlDuplicate
 ...
 '----------------------------------- ORGANIZ EL ORDEN DE LOS CAMPOS DE LA HOJA SALARIAL -----------------------------------
    ...
    'GrPer   Grupo de personal   ÁPers   Área de personal
    'Posición    Posición    Función Función DivP    División de personal
    'Clase de identificación (clase  Número ID
    Range("BC:BN").Select
    Selection.Copy
    Range("T:T").Select
    Selection.PasteSpecial Paste:=xlPasteValues, Operation:=xlNone, SkipBlanks _
    ...
    'CC  Clase de contrato   Soc.    Sociedad    Div.    División
    'S   Status ocupación
    Range("BR:CC").Select
    Selection.Copy
    Range("AF:AF").Select
    ...
    'VP  Vía de pago Clave banco Clave de banco
    'Áre Área de convenio
    Range("CH:CM").Select
    Selection.Copy
    Range("AR:AR").Select
    Selection.PasteSpecial Paste:=xlPasteValues, Operation:=xlNone, SkipBlanks _
